...
import pandas as pd
import SolveRD.script.b_script_get_gene.get_gene_from_case as NT_p # import all info for phenopacket nodetype df
import SolveRD.script.b_script_get_gene.get_gene_from_orpha as NT_orpha # import all info for phenopacket nodetype df
...
logger.info("START\t3_get_ALL_nodetype.py\n ")
...
with open(PATH_INPUT_PRODUCT_PD6, 'r',encoding="ISO-8859-1") as f:
    ...
    while ligne != "":
        ...
        # list_pd6.append(("orphanet", id_g))
        if source == "HGNC":
            list_pd6.add((symbol, id_g,reference))
            symbol = "symbol"
            id_g = "orphanet"
            reference = source = ""
        ligne = f.readline()

df_type_gene = pd.DataFrame(list_pd6, columns=['symbol',"orphanet",'HGNC'])

# verif
# df_type_gene.HGNC.value_counts()
...
####################################
logger.info('3_get_ALL_nodetype.py\texport the df NODE type orpha')
df_type_orpha.to_csv(PATH_OUPUT_NODE_TYPE_ORPHA, sep='\t',index=False)

logger.info('3_get_ALL_nodetype.py\texport the df NODE type genes')
df_type_gene.to_csv(PATH_OUPUT_NODE_TYPE_GENE, sep='\t',index=False)

logger.info('3_get_ALL_nodetype.py\texport the df NODE type phenopacket')
df_type_p.to_csv(PATH_OUPUT_NODE_TYPE_PHENO, sep='\t',index=False)

logger.info('3_get_ALL_nodetype.py\texport the df NODE type ERN')
df_ERN_filtered.to_csv(PATH_OUPUT_NODE_TYPE_ERN, sep='\t',index=False)

####################################
logger.info("END\t3_get_ALL_nodetype.py\n ")
