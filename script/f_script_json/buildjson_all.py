...
def set_variant(path, list_node, the_df_step_variant, case, T_step):
    ...
    df_I_V.to_csv(path + T_step + '_variant_' + str(
        case) + '.tsv', sep='\t')
...
def build_variant_AC_json(type_step, df_step_variant,nodetype_pheno,nodetype_ERN,nodetype_orpha,df_gene,cytoscapeWiki_folder):
    ...
    for cytoscapefile in cytoscapeWiki_folder:
        if type_step in cytoscapefile:
            ...
            df_info_variant = set_variant(PATH_OUTPUT_VARIANT,all_nodes,df_step_variant,removetsv,type_step)
            ...
            for onenode in set(all_nodes):
                ...
                if onenode in set(nodetype_pheno['case']):
                    ...
                    nodeinfo = nodetype_pheno[nodetype_pheno['case'] == onenode].values[0]  # it's a list ['P0002635', 'UNSOLVED']
                    if onenode in set(nodetype_ERN['element']):
                        nodeERN = nodetype_ERN[nodetype_ERN['element'] == onenode].values[0]  # it's a list ['P0002635', 'NMD']
                        ERNinfo = nodeERN[1]
                    else:
                        ERNinfo = "no"
    ...
    print(type_step, "\tINPUT cytsocape files size : \t", i)
...
if __name__ == '__main__':
    ...
    cyto_F = read_osfiles(PATH_OUTPUT_CYTOSCAPE_WIKIPATHWAY)

    try:
        # Create target Directory
         cyto_F.remove("cytoscape_homepage_ALL.tsv")
         print("Remove cytoscape_homepage_ALL.tsv  ")

    except :
         print(" cytoscape_homepage_ALL.tsv Already removed")

    list_proc = []
    for i  in range(len(tab_step)):
        print(tab_step[i],'\t',variant_step[i])
        P_step = Process(target=build_variant_AC_json, args=(tab_step[i],pd.read_excel(PATH_INPUT_VARIANT+variant_step[i], usecols=[2,8,37]),NT_pheno,NT_ERN,NT_orpha,NT_gene,cyto_F))
