...
input_cases = [i+'.json.json' for i in input_cases]

resultjsonjson = os.listdir(PATH_OUTPUT_RSLT_ALGO_PHENO)
match_jsonresult = set(input_cases).intersection(set(resultjsonjson))
...
while i < len(match_jsonresult):
    ...
    with open(PATH_OUTPUT_RSLT_ALGO_PHENO + str(
            match_jsonresult[i])) as file_phenopacket_result:
...
logger.info('homepage.py\tTIME : BUILD the df :{}'.format(datetime.datetime.utcnow()))
...
df_keep_best_interaction.to_csv(PATH_OUTPUT_CYTOSCAPE+"\cytoscape_homepage_ALL.tsv",sep='\t')
...
logger.info('homepage.py\tSTART BUILDING JSON')
...
logger.info('homepage.py\tedges+nodes in a dict')
...
logger.info('homepage.py\tEXPORT JSON')

with open(PATH_OUTPUT_JSON + str("\homepage_ALL.json"),'w') as f:
