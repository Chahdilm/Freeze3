...
input_json = "/json_filterHPO/"
output_json = "/json_curation_tmp/"
...
file_hpo = dplyr::select(file_hpo, -c(HPO.negated))

# We remove the wrong annotations
file_hpo = file_hpo[!file_hpo$HPO.type.id == file_hpo$HPO.type.label, ]
file_hpo = dplyr::filter(file_hpo, !is.na(file_hpo$HPO.type.label))
file_hpo = dplyr::filter(file_hpo, HPO.type.label != 'Invalid id')
...
phenopacket_hpo = dplyr::select(file_hpo, c(HPO.type.id, HPO.type.label))
phenopacket_hpo = dplyr::filter(phenopacket_hpo, !duplicated(phenopacket_hpo))
...
b = dplyr::filter(b, !duplicated(b[, 1:2]))
b = dplyr::select(b, c(Pid = Pid, HPO.type, HPO.label = HPO.type.label))

# We remove all the labels that are not in the HPO database
b = b %>% dplyr::arrange(Pid)
...
file_5HPO = dplyr::filter(HPO_count, HPO > 4)
...
to_remove =  dplyr::anti_join(all_file, file_5HPO, by = 'Pid')
